...
colnames(sel_alm_d)[1]<-"Probability"
sel_cot_d<-as.data.frame(sel_cot)
sel_cot_d<-na.omit(sel_cot_d)
sel_cot_d$crop<-"Cotton"
colnames(sel_cot_d)[1]<-"Probability"
sel_corn_d<-as.data.frame(sel_corn)
sel_corn_d<-na.omit(sel_corn_d)
sel_corn_d$crop<-"Corn"
colnames(sel_corn_d)[1]<-"Probability"
sel_pis_d<-as.data.frame(sel_pis)
sel_pis_d<-na.omit(sel_pis_d)
sel_pis_d$crop<-"Pistachios"
colnames(sel_pis_d)[1]<-"Probability"
sel_tom_d<-as.data.frame(sel_tom)
sel_tom_d<-na.omit(sel_tom_d)
sel_tom_d$crop<-"Tomatoes"
colnames(sel_tom_d)[1]<-"Probability"
sel_wal_d<-as.data.frame(sel_wal)
sel_wal_d<-na.omit(sel_wal_d)
sel_wal_d$crop<-"Walnuts"
colnames(sel_wal_d)[1]<-"Probability"
...
data_6<-rbind(sel_alm_d, sel_cot_d, sel_corn_d, sel_tom_d, sel_pis_d, sel_wal_d)
head(data_6)
str(data_6)
...
ggplot(aes(x = Probability, y = crop, fill = crop)) + 
 ...
 scale_fill_manual(values = ghibli_palette("MononokeMedium", 6))+
  theme(panel.background = element_blank(), 
