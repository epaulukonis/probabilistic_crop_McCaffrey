...
dim(datff) #2000000 pixels

#for each field, find average probabilities for each crop
sum_mat<-datff %>% group_by(ID) %>% summarise_all(funs(mean)) #summarize each crop by field to mean
dim(sum_mat) #16000

#calculate non-crop probability
sum_c <- apply(sum_mat[,c(2:30)], 1, sum) 
sum_mat$NC<-round(1-sum_c,4) #add in column for non-crop

#omit fields which don't overlap with crop data
sum_mat<-na.omit(sum_mat) 

#create empty data frame to hold the simulations
